...
Live provider execution remains opt-in. The current process environment reports a blocked billing/routing variable, so no authenticated provider round was attempted during V1 certification. The controller must continue to reject that environment rather than route through Platform API billing. M0.8 certification evidence for the individual seats is carried forward separately, but a current-host Tauri R1/R2/R3 round, human decision, and deterministic export remain explicit runtime gates. The NSIS installer and persisted local cancellation/restart path are certified separately in [V1-PRODUCTION-CERTIFICATION.md](../evidence/V1-PRODUCTION-CERTIFICATION.md).
