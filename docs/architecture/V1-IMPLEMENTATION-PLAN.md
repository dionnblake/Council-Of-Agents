...
Live provider execution remains opt-in. The effective provider command environment is validated independently from the ambient host: inherited credentials are excluded, `env_clear` is retained in the runner, and configured API-key or custom-routing values fail closed. M0.8 certification evidence for the individual seats is carried forward separately, but a current-host Tauri R1/R2/R3 round, human decision, and deterministic export remain explicit runtime gates. The NSIS installer and persisted local cancellation/restart path are certified separately in [V1-PRODUCTION-CERTIFICATION.md](../evidence/V1-PRODUCTION-CERTIFICATION.md).
